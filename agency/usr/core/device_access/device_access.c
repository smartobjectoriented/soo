...
void inject_MEs_from_filesystem(void) {
	...
	FILE *file;
	struct dirent *ent;
	char ME_file_name[256];
	char read_buf[1024];
	int nfread, ME_size, read_bytes;
	...
	while ((ent = readdir(directory)) != NULL) {
		...
		sprintf(ME_file_name, SOO_ME_DIRECTORY "/%s", ent->d_name);
		file = fopen(ME_file_name, "r");
		...
		/* Read the ME content  */
		while (read_bytes < ME_size) {
			nfread = fread(read_buf, 1, 1024, file);
			if (nfread < 0) {
				printf("Error when reading the ME\n");
				BUG();
			}
			memcpy(ME_buffer + read_bytes, read_buf, nfread);
			read_bytes += nfread;
		}
