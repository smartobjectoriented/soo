                                     ...
                                     int sl)
{
    ...
    {
        clear_bit(sl, (unsigned long *)&p->sl_bitmap[fl]);
        if ( !p->sl_bitmap[fl] )
            clear_bit(fl, (unsigned long *)&p->fl_bitmap);
    }
...
static inline void INSERT_BLOCK(struct bhdr *b, struct xmem_pool *p, int fl, int sl)
{
    ...
    p->matrix[fl][sl] = b;
    set_bit(sl, (unsigned long *)&p->sl_bitmap[fl]);
    set_bit(fl, (unsigned long *)&p->fl_bitmap);
}
    ...
    unsigned long grow_size)
{
    ...
    pool = (void *) alloc_heap_pages(pool_order, 0);
    if ( pool == NULL )
        return NULL;
...
void xmem_pool_destroy(struct xmem_pool *pool) 
{
    ...
    pool_bytes = ROUNDUP_SIZE(sizeof(*pool));
    pool_order = get_order_from_bytes(pool_bytes);
    free_heap_pages(pool,pool_order);
}
...
static struct xmem_pool *mempool;

static void *xmalloc_pool_get(unsigned long size)
{
    ASSERT(size == PAGE_SIZE);
    return alloc_heap_page();
}
...
static void *xmalloc_whole_pages(unsigned long size)
{
    ...
    b = alloc_heap_pages(pageorder, 0);
    if ( b == NULL )
        return NULL;
...
static void tlsf_init(void)
{
    ...
    mempool = xmem_pool_create("xmalloc", xmalloc_pool_get, xmalloc_pool_put, PAGE_SIZE, 0, PAGE_SIZE);
    BUG_ON(!mempool);
}
...
void *_xmalloc(unsigned long size, unsigned long align)
{
    ...
    if ( size < PAGE_SIZE )
        p = xmem_pool_alloc(size, mempool);
    if ( p == NULL )
...
void xfree(void *p)
{
    ...
    if ( b->size >= PAGE_SIZE )
        free_heap_pages((void *)b, get_order_from_bytes(b->size));
    else
        xmem_pool_free(p, mempool);
}
