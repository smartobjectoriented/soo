...
void __setup_dom_pgtable(struct domain *d, addr_t paddr_start, unsigned long map_size) {
	...
	create_mapping(new_pt, memslot[MEMSLOT_AGENCY].ipa_addr, paddr_start, map_size, false, S2);

	create_mapping(new_pt, IO_SPACE_PADDR, IO_SPACE_PADDR, IO_SPACE_SIZE, true, S2);

	/* Map the shared page in the IPA space; the shared page is located right after the domain area
	 * in the IPA space.
	 */
	create_mapping(new_pt, memslot[MEMSLOT_AGENCY].ipa_addr + map_size, virt_to_phys(d->avz_shared), PAGE_SIZE, true, S2);

	if (d->avz_shared->subdomain_shared)  {

		create_mapping(new_pt, memslot[MEMSLOT_AGENCY].ipa_addr + map_size + PAGE_SIZE,
			       virt_to_phys(d->avz_shared->subdomain_shared_paddr), PAGE_SIZE, true, S2);
