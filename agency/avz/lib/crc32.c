 ...
 */
uint32_t ZEXPORT crc32_no_comp(uint32_t crc, const unsigned char *buf, uInt len)
{
...
uint32_t ZEXPORT crc32 (uint32_t crc, const unsigned char *p, uInt len)
{
...
unsigned int xcrc32 (const unsigned char *buf, int len, unsigned int init)
{
  ...
  while (len--)
    {
      crc = (crc << 8) ^ crc_table[((crc >> 24) ^ *buf) & 255];
      buf++;
    }
  return crc;
}
