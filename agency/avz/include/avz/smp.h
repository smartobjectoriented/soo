#ifndef __SMP_H__
#define __SMP_H__

#include <avz/config.h>
#include <asm/smp.h>

/*
 * stops all CPUs but the current one:
 */
extern void smp_send_stop(void);

extern void set_smp_cross_call(void (*fn)(const struct cpumask *, unsigned int));
extern void smp_send_event_check_mask(const cpumask_t *mask);
#define smp_send_event_check_cpu(cpu) smp_send_event_check_mask(cpumask_of(cpu))

/*
 * Prepare machine for booting other CPUs.
 */
extern void smp_prepare_cpus(unsigned int max_cpus);

/*
 * Bring a CPU up
 */
extern int __cpu_up(unsigned int cpu);


/*
 * Final polishing of CPUs
 */
extern void smp_cpus_done(unsigned int max_cpus);

/*
 * Call a function on all other processors
 */
extern int smp_call_function(void (*func) (void *info), void *info, int wait);

/* 
 * Call a function on a selection of processors
 */
extern int on_selected_cpus(const cpumask_t *selected, void (*func) (void *info), void *info, int wait);

/*
 * Mark the boot cpu "online" so that it can call console drivers in
 * printk() and can access its per-cpu storage.
 */
void smp_prepare_boot_cpu(void);

extern void smp_init(void);

/*
 * Call a function on all processors
 */
static inline int on_each_cpu(void (*func) (void *info), void *info, int wait)
{
    return on_selected_cpus(&cpu_online_map, func, info, wait);
}


int alloc_cpu_id(void);

#endif /* __SMP_H__ */
