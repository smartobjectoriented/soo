...
#define irqoff_only()		XENO_BUG_ON(CONTEXT, irqs_disabled() == 0)
#if XENO_DEBUG(LOCKING)
#define atomic_only()		XENO_BUG_ON(CONTEXT, (xnlock_is_owner(&nklock) && irqs_disabled()) == 0)
#define preemptible_only()	XENO_BUG_ON(CONTEXT, xnlock_is_owner(&nklock) || irqs_disabled())
#else
#define atomic_only()		XENO_BUG_ON(CONTEXT, irqs_disabled() == 0)
#define preemptible_only()	XENO_BUG_ON(CONTEXT, irqs_disabled() != 0)
#endif
