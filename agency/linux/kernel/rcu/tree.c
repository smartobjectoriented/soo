...
{
	/* SOO.tech */
	/* If called from along the __purge_vmap_area_lazy() during a xnheap_vfree() call,
	 * we simply ignore the RCU handling when running on realtime agency CPU (#1).
	 */
...
static void __init rcu_init_one(void)
{
	...
	for_each_possible_cpu(i) {
		...
		/* SOO.tech */
		/* At this moment, the agency RT CPU is not out of possible CPUs mask. */
		if (i != AGENCY_RT_CPU) {
			while (i > rnp->grphi)
				rnp++;
