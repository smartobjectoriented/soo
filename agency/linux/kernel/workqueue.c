...
#if 0 /* SOO.tech - currently not used */
/**
...
static int worker_thread(void *__worker)
{
	...
	/* SOO.tech */
	/* No workqueue has to run on CPU other than CPU #0.
	 * Any work can be executed by the worker running on CPU #0.
	 */
	BUG_ON(smp_processor_id() != AGENCY_CPU);
...
void print_worker_info(const char *log_lvl, struct task_struct *task)
{
	...
	if (fn || name[0] || desc[0]) {
		/* SOO.tech */
		/* printk -> lprintk */
		lprintk("%sWorkqueue: %s %ps", log_lvl, name, fn);
		if (strcmp(name, desc))
			pr_cont(" (%s)", desc);
		pr_cont("\n");
	}
