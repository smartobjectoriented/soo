...
#include <soo/uimgr/socketmgr.h>

/* TCP/IP port on which the media server will listen to requests from smartphone/tablet */
...
void mediamgr_sendto(void *buff, size_t len) {
 ...
 */
static int gui_net_server_task_fn(void *arg) {
...
void mediamgr_init(void) {
	mutex_init(&sock_lock);

	kthread_run(gui_net_server_task_fn, NULL, "media_server");
}
