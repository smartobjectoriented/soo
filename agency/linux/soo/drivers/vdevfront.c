...
bool vdevfront_processing_begin(struct vbus_me_device *vdev) {
	vdevfront_t *vdevfront = (vdevfront_t *) vdev->vdrv->priv;;
 ...
 *
 */
static void __probe(struct vbus_me_device *vdev) {
	vdevfront_t *vdevfront = (vdevfront_t *) vdev->vdrv->priv;
 ...
 */
static void __otherend_changed(struct vbus_me_device *vdev, enum vbus_state backend_state) {
	vdevfront_t *vdevfront = (vdevfront_t *) vdev->vdrv->priv;
	vdrvfront_t *vdrvfront = to_vdrvfront(vdev);
...
int __shutdown(struct vbus_me_device *vdev) {
	vdevfront_t *vdevfront = (vdevfront_t *) vdev->vdrv->priv;
	vdrvfront_t *vdrvfront = to_vdrvfront(vdev);
