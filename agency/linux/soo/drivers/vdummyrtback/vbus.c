...
static bool processing_start(domid_t domid) {
	...
	if (!vdummyrt.vdev[domid] || ((vdev_id != 0) && (vdummyrt.vdev[domid]->id != vdev_id[domid]))) {
		
		/* Keep track of this instance - Help to maintain the consistency during processing in case
	 	 * of ME swap within a same slot.
	 	 */
		if (vdummyrt.vdev[domid])
			vdev_id[domid] = vdummyrt.vdev[domid]->id;
...
static void setup_sring(struct vbus_device *dev) {
	...
	vdummyrt_ring_t *p_vdummyrt_ring;

	p_vdummyrt_ring = &vdummyrt.rings[dev->otherend_id];

	rtdm_vbus_gather(VBT_NIL, dev->otherend, "ring-ref", "%lu", &ring_ref, "ring-evtchn", "%u", &evtchn, NULL);

	DBG("BE: ring-ref=%u, ring-evtchn=%u\n", ring_ref, evtchn);
	...
	BACK_RING_INIT(&p_vdummyrt_ring->ring, sring, PAGE_SIZE);

	res = rtdm_bind_interdomain_evtchn_to_virqhandler(&p_vdummyrt_ring->irq_handle, dev->otherend_id, evtchn, vdummyrt_interrupt, 0, "vdummyrt-backend", dev);
	BUG_ON(res < 0);
}
...
static void free_sring(struct vbus_device *dev) {
	vdummyrt_ring_t *p_vdummyrt_ring = &vdummyrt.rings[dev->otherend_id];

	/* Prepare to empty all buffers */
	BACK_RING_INIT(&p_vdummyrt_ring->ring, (&p_vdummyrt_ring->ring)->sring, PAGE_SIZE);

	rtdm_unbind_from_virqhandler(&p_vdummyrt_ring->irq_handle);

	vbus_unmap_ring_vfree(dev, p_vdummyrt_ring->ring.sring);
	p_vdummyrt_ring->ring.sring = NULL;
}
...
static int __vdummyrt_probe(struct vbus_device *dev, const struct vbus_device_id *id) {
	DBG("%s: SOO RT dummy driver for testing\n", __func__);

	rtdm_mutex_lock(&processing_count_lock[dev->otherend_id]);
	vdummyrt.vdev[dev->otherend_id] = dev;
...
static void frontend_changed(struct vbus_device *dev, enum vbus_state frontend_state) {
	...
	case VbusStateClosing:
		...
		vdummyrt.vdev[dev->otherend_id] = NULL;
...
static int __vdummyrt_suspend(struct vbus_device *dev) {
	...
	vdummyrt_suspend(dev);

	return 0;
}

static int __vdummyrt_resume(struct vbus_device *dev) {
	/* Resume Step 1 */
	BUG_ON(__connected[dev->otherend_id]);
...
static struct vbus_driver vdummyrt_drv = {
	...
	.suspend = __vdummyrt_suspend,
	.resume = __vdummyrt_resume,
};
...
void vdummyrt_vbus_init(void) {
	...
	vbus_register_backend(&vdummyrt_drv);
}
