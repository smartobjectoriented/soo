...
ROXML_STATIC ROXML_INT inline void roxml_release_last(void *data)
{
	...
	while ((ptr->prev != NULL) && (ptr->prev->id != current->pid))
		ptr = ptr->prev;
...
ROXML_INT void *roxml_malloc(int size, int num, int type)
{
	...
	cell->next = kmalloc(sizeof(memory_cell_t), GFP_KERNEL);
	if (!cell->next)
		return NULL;
	...
	cell->id = current->pid;
	cell->occ = size;
	cell->ptr = kcalloc(num, size, GFP_KERNEL);
	head_cell.prev = cell;
