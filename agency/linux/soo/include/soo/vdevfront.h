...
struct vdrvfront {
	...
	void (*probe)(struct vbus_me_device *vdev);

	void (*reconfiguring)(struct vbus_me_device *vdev);
	void (*shutdown)(struct vbus_me_device *vdev);
	void (*closed)(struct vbus_me_device *vdev);
	void (*suspend)(struct vbus_me_device *vdev);
	void (*resume)(struct vbus_me_device *vdev);
	void (*connected)(struct vbus_me_device *vdev);

};
typedef struct vdrvfront vdrvfront_t;

static inline vdrvfront_t *to_vdrvfront(struct vbus_me_device *vdev) {
...
bool vdevfront_processing_begin(struct vbus_me_device *vdev);
void vdevfront_processing_end(struct vbus_me_device *vdev);
