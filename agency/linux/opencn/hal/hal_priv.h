...
#define SHMPTR(hal_user, offset)  ( (void *)( hal_user->hal_shmem_base + (offset)))

/* SHMOFF(ptr) converts 'ptr' to an offset from shared mem base.  */
#define SHMOFF(hal_user, ptr)     ( ((char *)(ptr)) - hal_user->hal_shmem_base)
...
/* offset 0 is reserved for a null-ish pointer, so SHMCHK(hal_shmem_base) is
   false by design */
#define SHMCHK(hal_user, ptr)  ( ((char *)(ptr)) > (hal_user->hal_shmem_base) && ((char *)(ptr)) < (hal_user->hal_shmem_base + HAL_SIZE))
...
*/
typedef struct hal_data_struct_t {
...
*/
extern hal_comp_t *halpr_find_comp_by_name(hal_user_t *hal_user, const char *name);
extern hal_pin_t *halpr_find_pin_by_name(hal_user_t *hal_user, const char *name);
extern hal_sig_t *halpr_find_sig_by_name(hal_user_t *hal_user, const char *name);
extern hal_param_t *halpr_find_param_by_name(hal_user_t *hal_user, const char *name);
extern hal_thread_t *halpr_find_thread_by_name(hal_user_t *hal_user, const char *name);
extern hal_funct_t *halpr_find_funct_by_name(hal_user_t *hal_user, const char *name);

/** Allocates a HAL component structure */
extern hal_comp_t *halpr_alloc_comp_struct(hal_user_t *hal_user);
...
*/
extern hal_comp_t *halpr_find_comp_by_id(hal_user_t *hal_user, int id);
...
/** The 'find_xxx_by_owner()' functions find objects owned by a specific
 ...
 return NULL.
 */
extern hal_pin_t *halpr_find_pin_by_owner(hal_user_t *hal_user, hal_comp_t * owner, hal_pin_t * start);
extern hal_param_t *halpr_find_param_by_owner(hal_user_t *hal_user, hal_comp_t * owner, hal_param_t * start);
extern hal_funct_t *halpr_find_funct_by_owner(hal_user_t *hal_user, hal_comp_t * owner, hal_funct_t * start);
...
*/
extern hal_pin_t *halpr_find_pin_by_sig(hal_user_t *hal_user, hal_sig_t * sig, hal_pin_t * start);
