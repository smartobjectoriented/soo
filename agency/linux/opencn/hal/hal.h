...
typedef struct {
	...
	int major, minor; /* Associated major & minor for this component */
	hal_stream_t stream;
	int channel;
	...
	/* If a process will attach to a new component, we keep a reference to the associated comp_id. */
	int comp_id;
	...
	/* Private data related to this hal_user */
	void *priv;
...
typedef struct {
    ...
    int comp_id;
    void *param;			/* some pointer to a generic parameter */

} hal_thread_t;

extern hal_user_t *__core_hal_user;
...
extern int hal_init(hal_user_t *hal_user, const char *name);
...
extern int hal_exit(hal_user_t *hal_user, int comp_id);
...
/** hal_malloc() allocates a block of memory from the main HAL
 ...
 fresh.
 */
extern void *hal_malloc(hal_user_t *hal_user, long int size);
 ...
 */
extern int hal_ready(hal_user_t *hal_user, int comp_id);

/** hal_comp_name() returns the name of the given component, or NULL
 if comp_id is not a loaded component
 */

extern char *hal_comp_name(hal_user_t *hal_user, int comp_id);
 ...
 */
extern int hal_set_lock(hal_user_t *hal_user, unsigned char lock_type);
...
extern unsigned char hal_get_lock(hal_user_t *hal_user);
 ...
 */
extern int hal_pin_bit_new(hal_user_t *hal_user, const char *name, hal_pin_dir_t dir, hal_bit_t **data_ptr_addr, int comp_id);
extern int hal_pin_float_new(hal_user_t *hal_user, const char *name, hal_pin_dir_t dir, hal_float_t **data_ptr_addr, int comp_id);
extern int hal_pin_u32_new(hal_user_t *hal_user, const char *name, hal_pin_dir_t dir, hal_u32_t **data_ptr_addr, int comp_id);
extern int hal_pin_s32_new(hal_user_t *hal_user, const char *name, hal_pin_dir_t dir, hal_s32_t **data_ptr_addr, int comp_id);
...
/** The hal_pin_XXX_newf family of functions are similar to
 ...
 On failure they return a negative error code.
 */
extern int hal_pin_bit_newf(hal_user_t *hal_user, hal_pin_dir_t dir, hal_bit_t **data_ptr_addr, int comp_id, const char *fmt, ...)
	__attribute__((format(printf,5,6)));

extern int hal_pin_float_newf(hal_user_t *hal_user, hal_pin_dir_t dir, hal_float_t **data_ptr_addr, int comp_id, const char *fmt, ...)
	__attribute__((format(printf,5,6)));

extern int hal_pin_u32_newf(hal_user_t *hal_user, hal_pin_dir_t dir, hal_u32_t **data_ptr_addr, int comp_id, const char *fmt, ...)
	__attribute__((format(printf,5,6)));

extern int hal_pin_s32_newf(hal_user_t *hal_user, hal_pin_dir_t dir, hal_s32_t **data_ptr_addr, int comp_id, const char *fmt, ...)
	__attribute__((format(printf,5,6)));

/* TODO: Add description */
extern int hal_pin_newfv(hal_user_t *hal_user, hal_type_t type, hal_pin_dir_t dir, void **data_ptr_addr, int comp_id, const char *fmt, va_list ap);
 ...
 */
extern int hal_pin_new(hal_user_t *hal_user, const char *name, hal_type_t type, hal_pin_dir_t dir, void **data_ptr_addr, int comp_id);

/**
 * Delete a specific pin identified by its name.
 */
extern int hal_pin_delete(hal_user_t *hal_user, const char *name);
 ...
 */
extern int hal_pin_alias(hal_user_t *hal_user, const char *pin_name, const char *alias);
...
/** 'hal_signal_new()' creates a new signal object.  Once a signal has
 ...
 it returns a negative error code.
 */
extern int hal_signal_new(hal_user_t *hal_user, const char *name, hal_type_t type);
 ...
 */
extern int hal_signal_delete(hal_user_t *hal_user, const char *name);
...
/** 'hal_link()' links a pin to a signal.  'pin_name' and 'sig_name' are
 ...
 negative error code.
 */
extern int hal_link(hal_user_t *hal_user, const char *pin_name, const char *sig_name);
...
/** 'hal_unlink()' unlinks any signal from the specified pin.  'pin_name'
 ...
 returns a negative error code.
 */
extern int hal_unlink(hal_user_t *hal_user, const char *pin_name);
...
/** The 'hal_param_xxx_new()' functions create a new 'parameter' object.
 ...
 On failure they return a negative error code.
 */
extern int hal_param_bit_new(hal_user_t *hal_user, const char *name, hal_param_dir_t dir, hal_bit_t * data_addr, int comp_id);
extern int hal_param_float_new(hal_user_t *hal_user, const char *name, hal_param_dir_t dir, hal_float_t * data_addr, int comp_id);
extern int hal_param_u32_new(hal_user_t *hal_user, const char *name, hal_param_dir_t dir, hal_u32_t * data_addr, int comp_id);
extern int hal_param_s32_new(hal_user_t *hal_user, const char *name, hal_param_dir_t dir, hal_s32_t * data_addr, int comp_id);

/** printf_style-style versions of hal_param_XXX_new */
extern int hal_param_bit_newf(hal_user_t *hal_user, hal_param_dir_t dir, hal_bit_t * data_addr, int comp_id, const char *fmt, ...) __attribute__((format(printf,5,6)));
extern int hal_param_float_newf(hal_user_t *hal_user, hal_param_dir_t dir, hal_float_t * data_addr, int comp_id, const char *fmt, ...) __attribute__((format(printf,5,6)));
extern int hal_param_u32_newf(hal_user_t *hal_user, hal_param_dir_t dir, hal_u32_t * data_addr, int comp_id, const char *fmt, ...) __attribute__((format(printf,5,6)));
extern int hal_param_s32_newf(hal_user_t *hal_user, hal_param_dir_t dir, hal_s32_t * data_addr, int comp_id, const char *fmt, ...) __attribute__((format(printf,5,6)));
...
/** 'hal_param_new()' creates a new 'parameter' object.  It is a generic
 ...
 it returns a negative error code.
 */
extern int hal_param_new(hal_user_t *hal_user, const char *name, hal_type_t type, hal_param_dir_t dir, void *data_addr, int comp_id);
 ...
 */
extern int hal_param_bit_set(hal_user_t *hal_user, const char *name, int value);
extern int hal_param_float_set(hal_user_t *hal_user, const char *name, double value);
extern int hal_param_u32_set(hal_user_t *hal_user, const char *name, unsigned long value);
extern int hal_param_s32_set(hal_user_t *hal_user, const char *name, signed long value);
 ...
 */
extern int hal_param_alias(hal_user_t *hal_user, const char *pin_name, const char *alias);
...
/** 'hal_param_set()' is a generic function that sets the value of a
 ...
 it returns a negative error code.
 */
extern int hal_param_set(hal_user_t *hal_user, const char *name, hal_type_t type, void *value_addr);
...
/** hal_export_funct() makes a realtime function provided by a
 ...
 realtime code.
 */
extern int hal_export_funct(hal_user_t *hal_user, const char *name, void (*funct)(void *, long), void *arg, int uses_fp, int reentrant, int comp_id);
 ...
 */
extern int hal_create_thread(hal_user_t *hal_user, const char *name, unsigned long period_nsec, int uses_fp);

/** hal_create_custom_thread() establishes a realtime thread that will
 execute one or more HAL functions periodically.
 The difference with hal_create_thread() is that the caller has to implement the
 "thread task". This "thread task" is passed as an argument. It is useful for
 thread for which the period has to be controlled (for example to eliminate jitter)
 The hal_execute_thread_func() function should be call to execute functions added
 ...
 be non-zero for most threads, with the possible exception
 of the very fastest, most critical thread in a system.
 'taskcode' is the thread function.
 'priority' thread priority
 'param' is a parameter passed to the thread.
 On success, hal_create_thread() returns a positive integer
 thread ID.  On failure, returns an error code as defined
 above.  Call only from realtime init code, not from user
 space or realtime code.

 note 1: The thread period is handled by the caller/thread creator.

 note 2: the following code show basic structure to create a custom:
	void thread_task(void *arg) {
		hal_thread_t *thread = (hal_thread_t *)arg;
		my_param_t *param = (my_param_t *)thread->param;

		while (1) {
			if (hal_is_threads_started(__core_hal_user)) {
				...
				hal_execute_thread_func();
				...
			}
 ...
 */
extern int hal_create_custom_thread(hal_user_t *hal_user, const char *name, int uses_fp, void (*taskcode)(void *), void *param);

/** hal_create_custom_thread() executes the function added to a thread created
 with hal_create_custom_thread() function.
 'thread' the thread
 */
extern void hal_execute_thread_func(hal_thread_t *thread);
...
/** hal_add_funct_to_thread() adds a function exported by a
 ...
 realtime code.
 */
extern int hal_add_funct_to_thread(hal_user_t *hal_user, const char *funct_name, const char *thread_name, int position);
...
/** hal_del_funct_from_thread() removes a function from a thread.
 ...
 realtime code.
 */
extern int hal_del_funct_from_thread(hal_user_t *hal_user, const char *funct_name, const char *thread_name);
...
/** hal_start_threads() starts all threads that have been created.
 ...
 error code.
 */
extern int hal_start_threads(hal_user_t *hal_user);
...
/** hal_stop_threads() stops all threads that were previously
 ...
 error code.
 */
extern int hal_stop_threads(hal_user_t *hal_user);


/** hal_is_threads_started() returns the info if all threads have been started or
    not.
 */
extern int hal_is_threads_started(hal_user_t *hal_user);
 ...
 */
extern int hal_set_constructor(hal_user_t *hal_user, int comp_id, constructor make);
...
/** clear shared FIFO */
extern void hal_stream_clear_fifo(hal_stream_t *stream);
...
extern void hal_stream_wait_readable(hal_stream_t *stream);
...
extern void hal_stream_wait_writable(hal_stream_t *stream);

hal_user_t *find_hal_user_by_dev(int major, int minor);
void add_hal_user(hal_user_t *hal_user);
