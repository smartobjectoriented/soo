...
void lcec_el1252_read(struct lcec_slave *slave, long period)
{
	...
	for (i=0; i<LCEC_EL1252_CHANS; i++) {
		...
		/* read inputs */
		in = EC_READ_BIT(&pd[chan->in_offs], chan->in_bitp);

		if (in != *(chan->in))
			rtapi_print_msg(RTAPI_MSG_INFO, LCEC_MSG_PFX "IN%d state change, current: %d\n", i, in);
...
int lcec_el1252_init(int comp_id, struct lcec_slave *slave, ec_pdo_entry_reg_t *pdo_entry_regs)
{
	...
	printk("== lcec_el1252_init called================================\n");

	/* initialize callbacks */
	slave->proc_read = lcec_el1252_read;

	/* alloc hal memory */
	if ((hal_data = hal_malloc(__core_hal_user, sizeof(lcec_el1252_data_t))) == NULL) {
