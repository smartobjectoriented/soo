...
/* TODO */
#if (CONFIG_OPENCN_ETHERCAT_MASTER_NR > 1)
#error "Multiple Ethercat master device is not supported"
#endif

static char *main_devices[MAX_MASTERS] = { CONFIG_OPENCN_ETHERCAT_MASTER_MAC0, }; /**< Main devices parameter. */
static unsigned int master_count = CONFIG_OPENCN_ETHERCAT_MASTER_NR;
static char *backup_devices[MAX_MASTERS]; /**< Backup devices parameter. */
static unsigned int backup_count = 0; /**< Number of backup devices. */
static unsigned int debug_level = CONFIG_OPENCN_ETHERCAT_MASTER_DEBUG_LEVEL; /**< Debug level parameter. */

static ec_master_t *masters; /**< Array of masters. */
static rtdm_sem_t master_sem; /**< Master semaphore. */
...
int __init ec_init_module(void) {
	...
	for (i = 0; i < master_count; i++) {
                /* OpenCN - allow empty MAC address - for example for rpi4 case */
		ret = ec_mac_parse(macs[i][0], main_devices[i], 1);
		if (ret)
			goto out_class;
	...
	if (master_count) {
		if (!(masters = kmalloc(sizeof(ec_master_t) * master_count, GFP_ATOMIC))) {
	...
	for (i = 0; i < master_count; i++) {
		ret = ec_master_init(&masters[i], i, macs[i][0], macs[i][1], device_number, class, debug_level);
		if (ret)
	...
	out_free_masters: for (i--; i >= 0; i--)
		ec_master_clear(&masters[i]);
	kfree(masters);
	out_class: class_destroy(class);
	out_cdev: if (master_count)
		unregister_chrdev_region(device_number, master_count);
	out_return: return ret;
}
...
/* OpenCN - for rpi4, no need to check the MAC address */
#if !defined(CONFIG_ARCH_BCM2835) && !defined(CONFIG_NET_VENDOR_CADENCE)
	unsigned int i;
 ...
 */
ssize_t ec_mac_print(const uint8_t *mac, /**< MAC address */
 ...
 */
int ec_mac_is_zero(const uint8_t *mac /**< MAC address. */
) {
	unsigned int i;
 ...
 */
int ec_mac_is_broadcast(const uint8_t *mac /**< MAC address. */
) {
	unsigned int i;
...
/** Device names.
 */
const char *ec_device_names[2] = { "main", "backup" };
...
ec_device_t *ecdev_offer(struct net_device *net_dev, ec_pollfunc_t poll, struct module *module) {
	...
	for (i = 0; i < master_count; i++) {
		...
		down(&master->nort_master_sem);

		for (dev_idx = EC_DEVICE_MAIN; dev_idx < ec_master_num_devices(master); dev_idx++) {
			if (!master->devices[dev_idx].dev
											&& (ec_mac_equal(master->macs[dev_idx],
																			net_dev->dev_addr)
																			|| ec_mac_is_broadcast(
																											master->macs[dev_idx]))) {
				EC_INFO("Accepting %s as %s device for master %u.\n", str, ec_device_names[dev_idx != 0],
												master->index);

				ec_device_attach(&master->devices[dev_idx], net_dev, poll, module);
				up(&master->nort_master_sem);

				snprintf(net_dev->name, IFNAMSIZ, "ec%c%u", ec_device_names[dev_idx != 0][0], master->index);
 ...
 */
ec_master_t *ecrt_request_master_err(unsigned int master_index /**< Master index. */) {
	...
	if (rtdm_sem_down(&master_sem)) {
		errptr = ERR_PTR(-EINTR);
		goto out_return;
	}

	if (master->reserved) {
		rtdm_sem_up(&master_sem);
	...
	if (rtdm_sem_down(&master->device_sem)) {
		errptr = ERR_PTR(-EINTR);
		goto out_release;
	}

	if (master->phase != EC_IDLE) {
		rtdm_sem_up(&master->device_sem);
	...
	out_module_put: for (; dev_idx > 0; dev_idx--) {
		ec_device_t *device = &master->devices[dev_idx - 1];
		module_put(device->module);
	}
	out_release: master->reserved = 0;
	out_return: return errptr;
}
 ...
 */
const ec_request_state_t ec_request_state_translation_table[] = { EC_REQUEST_UNUSED,  // EC_INT_REQUEST_INIT,
...
/* The RT subsystem is initialized, but not the e1000e driver yet. */
subsys_initcall_sync(ec_init_module)
