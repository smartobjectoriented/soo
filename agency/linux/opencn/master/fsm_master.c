    ...
    if (fsm->rescan_required) {
        ...
        if (!master->allow_scan) {
            ...
            if (!count) {
                ...
                master->scan_busy = 0;
                rtdm_waitqueue_signal(&master->scan_queue);
            ...
            if (!(master->slaves =
                        (ec_slave_t *) kmalloc(size, GFP_ATOMIC))) {
        ...
        )
{
    ...
    for (slave = master->slaves;
        ...
        if (!(slave->sii.mailbox_protocols & EC_MBOX_COE)
                ...
                || rtdm_jiffies - slave->jiffies_preop < EC_WAIT_SDO_DICT * RTDM_HZ
                ) continue;
        ...
        )
{
    ...
    if (slave->sii.mailbox_protocols & EC_MBOX_EOE) {
        ...
        ec_eoe_t *eoe;
        if (!(eoe = kmalloc(sizeof(ec_eoe_t), GFP_ATOMIC))) {
    ...
    EC_MASTER_INFO(master, "Bus scanning completed in %lu ms.\n",
            (rtdm_jiffies - fsm->scan_jiffies) * 1000 / RTDM_HZ);
        ...
        )
{
    ...
    rtdm_waitqueue_signal(&master->config_queue);
        ...
        )
{
    ...
    correction = jiffies_since_read * 1000 / RTDM_HZ * 1000000;
        ...
        )
{
    ...
    // correct read system time by elapsed time since read operation
    correction = (u64) (jiffies_since_read * 1000 / RTDM_HZ) * 1000000;
        ...
        )
{
    ...
    jiffies_since_read = rtdm_jiffies - datagram->jiffies_sent;
        ...
        )
{
    ...
    if (!ec_fsm_coe_success(&fsm->fsm_coe)) {
        ...
        request->state = EC_INT_REQUEST_FAILURE;
        rtdm_waitqueue_broadcast(&fsm->master->request_queue);
        ec_fsm_master_restart(fsm);
        return;
    }
