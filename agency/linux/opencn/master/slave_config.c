        ...
        )
{
    ...
    rtdm_sem_down(&sc->master->master_sem);
    ec_fmmu_config_init(fmmu, sc, domain, sync_index, dir);
    rtdm_sem_up(&sc->master->master_sem);
        ...
        uint8_t subindex, const uint8_t *data, size_t size)
{
    ...
    if (!(req = (ec_sdo_request_t *)
          kmalloc(sizeof(ec_sdo_request_t), GFP_ATOMIC))) {
        ...
        ec_slave_config_t *sc, size_t size)
{
    ...
    if (!(reg = (ec_reg_request_t *)
                kmalloc(sizeof(ec_reg_request_t), GFP_ATOMIC))) {
        ...
        ec_slave_config_t *sc, size_t size)
{
    ...
    if (!(voe = (ec_voe_handler_t *)
                kmalloc(sizeof(ec_voe_handler_t), GFP_ATOMIC))) {
        ...
        size_t size)
{
    ...
    if (!(req = (ec_soe_request_t *)
          kmalloc(sizeof(ec_soe_request_t), GFP_ATOMIC))) {
        EC_CONFIG_ERR(sc, "Failed to allocate memory for"
                " IDN configuration!\n");
        return -ENOMEM;
    }
