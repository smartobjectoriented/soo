    ...
    if (datagram->working_counter == 0) {
        ...
        if (diff >= RTDM_HZ) {
            ...
            EC_SLAVE_DBG(slave, 1, "Resending after %u ms...\n",
                    (unsigned int) diff * 1000 / RTDM_HZ);
        }
        ...
        )
{
    ...
    if (config->dc_assign_activate) {
        ...
        EC_WRITE_U32(datagram->data + 4, config->dc_sync[1].cycle_time + config->dc_sync[1].shift_time);
        fsm->retries = EC_FSM_RETRIES;
        ...
        )
{
    ...
    fsm->jiffies_start = rtdm_jiffies;
        ...
        )
{
    ...
    diff_ms = (datagram->jiffies_received - fsm->jiffies_start) * 1000 / RTDM_HZ;
