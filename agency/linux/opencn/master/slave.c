...
void ec_slave_clear(ec_slave_t *slave /**< EtherCAT slave */)
{
    ...
    rtdm_waitqueue_broadcast(&slave->master->request_queue);
    ...
    if (slave->sii.string_count) {
        ...
        for (i = 0; i < slave->sii.string_count; i++) {
            ...
            if (!(slave->sii.strings[i] =
                        kmalloc(sizeof(char) * size + 1, GFP_ATOMIC))) {
        ...
        )
{
    ...
    if (count) {
        ...
        if (!(syncs = kmalloc(memsize, GFP_ATOMIC))) {
        ...
        )
{
    ...
    while (data_size >= 8) {
        if (!(pdo = kmalloc(sizeof(ec_pdo_t), GFP_ATOMIC))) {
        ...
        for (i = 0; i < entry_count; i++) {
            if (!(entry = kmalloc(sizeof(ec_pdo_entry_t), GFP_ATOMIC))) {
