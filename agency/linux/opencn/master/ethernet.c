        ...
        )
{
    ...
    rtdm_sem_init(&eoe->tx_queue_sem, 1);
...
void ec_eoe_flush(ec_eoe_t *eoe /**< EoE handler */)
{
    ...
    rtdm_sem_down(&eoe->tx_queue_sem);
...
void ec_eoe_run(ec_eoe_t *eoe /**< EoE handler */)
{
    ...
    // update statistics
    if (rtdm_jiffies - eoe->rate_jiffies > RTDM_HZ) {
        eoe->rx_rate = eoe->rx_counter;
        eoe->tx_rate = eoe->tx_counter;
        eoe->rx_counter = 0;
        eoe->tx_counter = 0;
        eoe->rate_jiffies = rtdm_jiffies;
    }
