        ...
        )
{
    ...
    if (!(datagram_pair = kmalloc(sizeof(ec_datagram_pair_t), GFP_ATOMIC))) {
        ...
        )
{
    ...
    if (domain->data_size && domain->data_origin == EC_ORIG_INTERNAL) {
        if (!(domain->data = (uint8_t *) kmalloc(domain->data_size, GFP_ATOMIC))) {
...
void ecrt_domain_external_memory(ec_domain_t *domain, uint8_t *mem)
{
    ...
    rtdm_sem_down(&domain->master->master_sem);
    ...
    rtdm_sem_up(&domain->master->master_sem);
}
...
void ecrt_domain_process(ec_domain_t *domain)
{
    ...
    if (domain->working_counter_changes &&
        rtdm_jiffies - domain->notify_jiffies > RTDM_HZ) {
        domain->notify_jiffies = rtdm_jiffies;
...
void ecrt_domain_queue(ec_domain_t *domain)
{
	...
	memcpy(datagram_pair->send_buffer, datagram_pair->datagrams[EC_DEVICE_MAIN].data, datagram_pair->datagrams[EC_DEVICE_MAIN].data_size);
#endif
	ec_master_queue_datagram(domain->master, &datagram_pair->datagrams[EC_DEVICE_MAIN]);

	/* copy main data to backup datagram */
	for (dev_idx = EC_DEVICE_BACKUP; dev_idx < ec_master_num_devices(domain->master); dev_idx++) {
		memcpy(datagram_pair->datagrams[dev_idx].data, datagram_pair->datagrams[EC_DEVICE_MAIN].data, datagram_pair->datagrams[EC_DEVICE_MAIN].data_size);
		ec_master_queue_datagram(domain->master, &datagram_pair->datagrams[dev_idx]);
        }
    }
}
