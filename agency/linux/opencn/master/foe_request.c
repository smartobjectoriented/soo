        ...
        )
{
    ...
    if (!(req->buffer = (uint8_t *) kmalloc(size, GFP_ATOMIC))) {
        ...
        )
{
    return req->issue_timeout
        && rtdm_jiffies - req->jiffies_start > RTDM_HZ * req->issue_timeout / 1000;
}
        ...
        )
{
    ...
    req->result = FOE_BUSY;
    req->jiffies_start = rtdm_jiffies;
}
