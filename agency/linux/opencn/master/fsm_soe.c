        ...
        )
{
    ...
    fsm->request->jiffies_sent = rtdm_jiffies;
        ...
        )
{
    ...
    diff_ms = (rtdm_jiffies - fsm->request->jiffies_sent) * 1000 / RTDM_HZ;
        ...
        )
{
    ...
    if (!ec_slave_mbox_check(fsm->datagram)) {
        unsigned long diff_ms =
            (datagram->jiffies_received - fsm->jiffies_start) * 1000 / RTDM_HZ;
        if (diff_ms >= EC_SOE_RESPONSE_TIMEOUT) {
            fsm->state = ec_fsm_soe_error;
