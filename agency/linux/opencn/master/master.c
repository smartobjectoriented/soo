...
#include <opencn/master/globals.h>
...
#ifdef EC_EOE
static void ec_master_eoe_thread(void *);
#endif
...
void ec_master_init_static(void)
{
    ...
    timeout_jiffies = max(EC_IO_TIMEOUT * RTDM_HZ / 1000000, 1);
    ext_injection_timeout_jiffies =
        max(EC_SDO_INJECTION_TIMEOUT * RTDM_HZ / 1000000, 1);
#endif
}
        ...
        )
{
    ...
    /* Perform an initialization of the nort semaphore used during e1000e_probe() */
    sema_init(&master->nort_master_sem, 1);

    rtdm_sem_init(&master->master_sem, 1);
    ...
    rtdm_sem_init(&master->device_sem, 1);
    ...
    master->scan_busy = 0;
    master->allow_scan = 1;
    rtdm_sem_init(&master->scan_sem, 1);
    rtdm_waitqueue_init(&master->scan_queue);

    master->config_busy = 0;
    rtdm_sem_init(&master->config_sem, 1);
    rtdm_waitqueue_init(&master->config_queue);

    INIT_LIST_HEAD(&master->datagram_queue);
    master->datagram_index = 0;

    INIT_LIST_HEAD(&master->ext_datagram_queue);
    rtdm_sem_init(&master->ext_queue_sem, 1);
    ...
    ec_master_set_send_interval(master, 1000000 / RTDM_HZ);
    ...
    rtdm_waitqueue_init(&master->request_queue);
    ...
    master->class_device = device_create(class, NULL, MKDEV(MAJOR(device_number), master->index), NULL, "EtherCAT%u",
									master->index);
...
#if 0 /* opencn - no interface with user space */
#ifdef EC_RTDM
    // init RTDM device
...
void ec_master_clear_slaves(ec_master_t *master)
{
    ...
    while (!list_empty(&master->sii_requests)) {
        ...
        rtdm_waitqueue_broadcast(&master->request_queue);
    }
...
{
    rtdm_sem_down(&master->master_sem);
    ec_master_clear_domains(master);
    ec_master_clear_slave_configs(master);
    rtdm_sem_up(&master->master_sem);
}
        ...
        )
{
    ...
    rtdm_sem_down(&master->io_sem);
    ecrt_master_send_ext(master);
    rtdm_sem_up(&master->io_sem);
}
        ...
        )
{
    ...
    ret = rtdm_task_init(&master->thread, name, thread_func, (void * ) master,
            CONFIG_OPENCN_ETHERCAT_MASTER_MASTER_THREAD_PRIORITY, 0);
    if (ret) {
        ...
        )
{
    ...
    sleep_jiffies = max(RTDM_HZ / 100, 1); // 10 ms, at least 1 jiffy

    rtdm_task_sleep(sleep_jiffies * SECONDS(1) / RTDM_HZ);
}
        ...
        )
{
    ...
    rtdm_sem_down(&master->config_sem);
    if (master->config_busy) {
        rtdm_sem_up(&master->config_sem);

        // wait for slave configuration to complete
        ret = rtdm_wait_condition(&master->config_queue,
                    !master->config_busy);
        if (ret) {
    ...
    rtdm_sem_down(&master->scan_sem);
    master->allow_scan = 0; // 'lock' the slave list
    if (!master->scan_busy) {
        rtdm_sem_up(&master->scan_sem);
    } else {
        rtdm_sem_up(&master->scan_sem);

        // wait for slave scan to complete
        ret = rtdm_wait_condition(&master->scan_queue,
                !master->scan_busy);
        if (ret) {
        ...
        )
{
    ...
    while (master->ext_ring_idx_rt != master->ext_ring_idx_fsm) {
...
#else
            if (rtdm_jiffies - datagram->jiffies_sent > ext_injection_timeout_jiffies)
#endif
            {
...
#else
                time_us = (unsigned int) ((rtdm_jiffies - datagram->jiffies_sent) * 1000000 / RTDM_HZ);
#endif
        ...
        master->ext_ring_idx_rt = (master->ext_ring_idx_rt + 1) % EC_EXT_RING_SIZE;
    }
        ...
        )
{
    ...
    master->max_queue_size = (send_interval * 1000) / EC_BYTE_TRANSMISSION_TIME_NS;
...
{
    rtdm_sem_down(&master->ext_queue_sem);
    list_add_tail(&datagram->queue, &master->ext_datagram_queue);
    rtdm_sem_up(&master->ext_queue_sem);
}
    ...
    do {
        ...
        list_for_each_entry(datagram, &master->datagram_queue, queue) {
            ...
            if (!frame_data) {
                ...
                frame_data = ec_device_tx_data(&master->devices[device_index]);
        ...
        // send frame
        ec_device_send(&master->devices[device_index], cur_data - frame_data);

#ifdef EC_HAVE_CYCLES
	cycles_sent = get_cycles();
#endif
        jiffies_sent = rtdm_jiffies;
...
{
    if (unlikely(rtdm_jiffies - master->stats.output_jiffies >= RTDM_HZ)) {
        master->stats.output_jiffies = rtdm_jiffies;
        ...
        )
{
    ...
    // frame statistics
    if (likely(rtdm_jiffies - s->jiffies < RTDM_HZ)) {
        return;
    }
...
static void ec_master_idle_thread(void *priv_data)
{
    ...
    while (!rtdm_task_should_stop()) {
        ...
        if (rtdm_sem_down(&master->master_sem)) {
            break;
        }
...
#else
            rtdm_task_sleep(sent_bytes * EC_BYTE_TRANSMISSION_TIME_NS);
#endif
        }
    }
...
void ec_master_eoe_start(ec_master_t *master /**< EtherCAT master */)
{
    ...
    err = rtdm_task_init(master->eoe_thread, "EtherCAT-EoE", ec_master_eoe_thread, (void * )master,
            CONFIG_OPENCN_ETHERCAT_MASTER_EOE_THREAD_PRIORITY,
            CONFIG_OPENCN_ETHERCAT_MASTER_EOE_THREAD_PERIOD_NS);
    if (err) {
...
void ec_master_eoe_stop(ec_master_t *master /**< EtherCAT master */)
{
    ...
    rtdm_task_destroy(master->eoe_thread);
 ...
 */
static void ec_master_eoe_thread(void *priv_data)
{
    ec_master_t *master = (ec_master_t *) priv_data;
    ec_eoe_t *eoe;
...
schedule:
        ...
        if (ret) {
            EC_MASTER_ERR(master, "rtdm_task_wait_period failed: %d\n", ret);
            break;
        }
    }
        ...
        )
{
    ...
    if (!(domain = (ec_domain_t *) kmalloc(sizeof(ec_domain_t), GFP_ATOMIC))) {
...
int ecrt_master_activate(ec_master_t *master)
{
...
#ifdef EC_EOE
    /* eoe_was_running = master->eoe_thread != NULL; */ //FIXME TODO
    ec_master_eoe_stop(master);
#endif
    ...
    ret = ec_master_thread_start(master, ec_master_operation_thread, "EtherCAT-OP");
    if (ret < 0) {
...
#endif
    if (ec_master_thread_start(master, ec_master_idle_thread, "EtherCAT-IDLE")) {
    ...
    list_for_each_entry_safe(datagram, next, &master->datagram_queue, queue) {
        ...
        if (master->devices[EC_DEVICE_MAIN].jiffies_poll -
            ...
            if (unlikely(master->debug_level > 0)) {
                ...
                time_us = (unsigned int)
                    ((master->devices[EC_DEVICE_MAIN].jiffies_poll -
                            datagram->jiffies_sent) * 1000000 / RTDM_HZ);
#endif
...
{
    ec_slave_config_t *sc = ecrt_master_slave_config_err(master, alias, position, vendor_id, product_code);
        ...
        size_t data_size, uint32_t *abort_code)
{
    ...
    if (rtdm_wait_condition(&master->request_queue,
    ...
    rtdm_wait_condition(&master->request_queue, request.state != EC_INT_REQUEST_BUSY);
