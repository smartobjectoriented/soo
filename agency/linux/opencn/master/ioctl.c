        ...
        )
{
    ...
    if (down_interruptible(&master->nort_master_sem)) {
        return -EINTR;
   }
...
static ec_ioctl_slave_soe_rt_read_t idn_data;

/* Read an SoE IDN from RT domain.
 */
void ecrt_ioctl_soe_rt_read(dc_event_t dc_event)
{
    ec_ioctl_slave_soe_read_t *ioctl = idn_data.ioctl;

    *idn_data.retval = ecrt_master_read_idn(idn_data.master,
    ...
    tell_dc_stable(DC_EC_IOCTL_SOE_READ);
}
        ...
        )
{
    ...
    /* Prepare the idn_data before the dc_event call */
    idn_data.master = master;
    idn_data.retval = &retval;
    idn_data.data   = data;
    idn_data.ioctl  = &ioctl;

    /* DC event call, perform SoE read in RT domain */
    do_sync_dom(DOMID_AGENCY_RT, DC_EC_IOCTL_SOE_READ);
