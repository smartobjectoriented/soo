...
/* This file contains data structures that live in shared memory and
   ...
   and kernel comps.  If the structure layouts used by various
   programs don't match, that's bad.  So we have revision checking.
   Whenever a comp or program is loaded, the rev_code is checked
...
typedef struct {
    ...
    int rtusers;		/* number of realtime comps using block */
    int ulusers;		/* number of user processes using block */
    unsigned long size;		/* size of shared memory area */
    unsigned long bitmap[(RTAPI_MAX_SHMEMS / 8) + 1];	/* which comps are
							   using block */
} shmem_data;

typedef struct {
    int users;			/* number of comps using the semaphore */
    int key;			/* key to semaphore */
    unsigned long bitmap[(RTAPI_MAX_SEMS / 8) + 1];	/* which comps are
							   using sem */
} sem_data;
...
typedef struct {
    ...
    int key;			/* key to fifo */
    int reader;			/* comp ID of reader */
    int writer;			/* comp ID of writer */
...
typedef struct {
    ...
    int rt_comp_count;		/* loaded RT comps */
    int ul_comp_count;		/* running UL processes */
    ...
    comp_data comp_array[RTAPI_MAX_COMP + 1];	/* data for comps */
...
static void init_rtapi_data(rtapi_data_t * data)
{
    ...
    /* set version code so other comps can check it */
    data->rev_code = rev_code;

    /* and get busy */
    data->rt_comp_count = 0;
    data->ul_comp_count = 0;
    data->task_count = 0;
    data->shmem_count = 0;
    data->sem_count = 0;
    data->fifo_count = 0;
	...
	data->comp_array[n].state = EMPTY;
	data->comp_array[n].name[0] = '\0';
    }
