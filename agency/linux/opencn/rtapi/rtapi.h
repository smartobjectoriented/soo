...
/** 'rtapi_init() sets up the RTAPI.  It must be called by any
    ...
    from realtime tasks.
*/
    extern int rtapi_comp_init(const char *compname);
...
*/
    extern int rtapi_comp_exit(int module_id);
...
/** 'rtapi_task_new()' creates but does not start a realtime task.
    ...
    is making the call (see rtapi_init).  'uses_fp' is a flag that
...
#define RTAPI_USES_FP 1
	...
	int prio, int owner, int uses_fp);
