...
struct gpio_chip bcm2835_gpio_chip = {
	.label = MODULE_NAME,
	.owner = THIS_MODULE,
...
struct irq_chip bcm2835_gpio_irq_chip = {
	.name = MODULE_NAME,
...
#if 0 /* used to dump the registers */
static int print_reg(void *data)
{
	struct bcm2835_pinctrl *pc = (struct bcm2835_pinctrl *) data;
	while(1) {

		printk("GPEDS0: 0x%08X\n", bcm2835_gpio_rd(pc, GPEDS0));
		printk("GPREN0: 0x%08X\n", bcm2835_gpio_rd(pc, GPREN0));
		msleep(1000);
	}
}
#endif	
