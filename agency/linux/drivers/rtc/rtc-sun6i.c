...
static int sun6i_rtc_probe(struct platform_device *pdev)
{
	...
	/* SOO.tech */
	/* Disable gating of clk out */
	tmp_data = readl(chip->base + SUN6I_LOSC_OUT_GATING);
	tmp_data |= (0x1);
	writel(tmp_data, chip->base + SUN6I_LOSC_OUT_GATING);
	dev_info(&pdev->dev, "gating of X32KFOUT disabled\n");

	gpio = ioremap(0x01f02c00, 0x200);

	/* set data 0 */
	tmp = ioread32(gpio + 0x10);
	tmp &= ~(0x1 << 6);
	iowrite32(tmp, gpio + 0x0);

	/* direction output */
	tmp = ioread32(gpio + 0x0);
	tmp &= ~(0x5 << 24);
	tmp |= (0x1 << 24);
	iowrite32(tmp, gpio + 0x0);
	...
	iounmap(gpio);
	dev_info(&pdev->dev, "reset wifi\n");  /* Wifi ? */

	/* End of SOO.tech add-on */
