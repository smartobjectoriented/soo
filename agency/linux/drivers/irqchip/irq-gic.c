...
/* SOO.tech */
struct irq_domain *__irq_domain = NULL;
			    ...
			    bool force)
{
	...
	u32 val, mask, bit;
	unsigned long flags;
	struct cpumask __mask_val;

	cpumask_copy(&__mask_val, mask_val);

	/* SOO.tech */
	/*
	 * First make sure the agency RT CPU is not present in the mask.
	 */
	cpumask_clear_cpu(AGENCY_RT_CPU, &__mask_val);

	if (!force)
		cpu = cpumask_any_and(&__mask_val, (struct cpumask *) cpu_online_mask);
	else
		cpu = cpumask_first(&__mask_val);
				...
				irq_hw_number_t hw)
{
	...
	if (hw < 32) {
		...
		cpumask_set_cpu(AGENCY_CPU, &__cpumask);

		irq_set_affinity(irq, &__cpumask);
	} else {
		irq_domain_set_info(d, irq, hw, &gic->chip, d->host_data,
			  ...
			  struct fwnode_handle *handle)
{
	...
	/* SOO.tech */
	BUG_ON(__irq_domain != NULL);
	__irq_domain = gic->domain;
