		  ...
		  int budget)
{
	...
	while (count < budget) {
		...
		if (bp->ecdev)
			ecdev_receive(bp->ecdev, skb->data, len);
		else
...
static int macb_poll(struct napi_struct *napi, int budget)
{
	...
	/* OpenCN - polling done in ec_poll */
	BUG();
...
static int macb_init(struct platform_device *pdev)
{
	...
	for (hw_q = 0, q = 0; hw_q < MACB_MAX_QUEUES; ++hw_q) {
		...
		/* OpenCN - No netif_* calls in EtherCAT driver */
		if (!bp->ecdev)
		 ...
		 * hardware queue mask.
		 */
		/* OpenCN - No irq in EtherCAT driver */
		if (!bp->ecdev) {
...
static void ec_poll(struct net_device *netdev)
{
	...
	gem_rx(queue, &queue->napi, UINT_MAX);
...
static int macb_probe(struct platform_device *pdev)
{
...
#if 0 /* OpenCN - no Power management */
	...
	/* OpenCN - 'offer' etherCAT device */
	bp->ecdev = ecdev_offer(dev, ec_poll, THIS_MODULE);
	if (bp->ecdev) {
		err = ecdev_open(bp->ecdev);
		if (err)
			ecdev_withdraw(bp->ecdev);

		ecdev_set_link(bp->ecdev, 0);
	...
	/* OpenCN - avoid netif_* calls in EtherCAT driver */
	if (!bp->ecdev)
		netif_carrier_off(dev);

	/* OpenCN - no differ work (all handled by Xenomai) */
	if (!bp->ecdev)
