...
#define pr_fmt(fmt)				"bcmgenet (ethercat): " fmt
...
{
	strlcpy(info->driver, "bcmgenet (ethercat)", sizeof(info->driver));
...
#if 0 /* e2cat - avoid netif_* calls in EtherCAT driver */
...
static void bcmgenet_tx_reclaim_all(struct net_device *dev)
{
	...
	/* e2cat - only DESC_INDEX queue used */
	if (!priv->ecdev) {
...
static netdev_tx_t bcmgenet_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (index == 0) {
		...
		/* e2cat - all data are sent/received on the Unclassified queue ! */
		BUG();
		index -= 1;
	}
...
#else
			printk("bcmgenet (ethercat): Tx DMA map failed\n");
#endif
			ret = NETDEV_TX_OK;
...
#else
		printk("bcmgenet (ethercat): Rx skb allocation failed\n");
#endif
		return NULL;
	}
...
#else
		printk("bcmgenet (ethercat): Rx skb DMA mapping failed\n");
#endif
		return NULL;
	}
...
#else
			printk("bcmgenet (ethercat): dropping fragmented packet!\n");
#endif
			ring->errors++;
			dev_kfree_skb_any(skb);
			goto next;
				 ...
				 unsigned int start_ptr, unsigned int end_ptr)
{
	...
	/* Initialize Rx NAPI */
	if (!priv->ecdev)
		netif_napi_add(priv->dev, &ring->napi, bcmgenet_rx_poll, NAPI_POLL_WEIGHT);
...
#if 0 /* e2cat - No IRQ in EtherCAT driver */
static void bcmgenet_irq_task(struct work_struct *work)
{
	...
	/* e2cat - no irq in EtherCAT driver */
	BUG();
...
#if 0 /* e2cat - Wake on Lan is disabled */
...
static int bcmgenet_open(struct net_device *dev)
{
	...
	/* e2cat - no IRQ with EtherCAT driver */
	if (!priv->ecdev) {
...
/* JMI check - It should not be done - correct ? */
#if 0 /* e2cat */
...
static void bcmgenet_timeout(struct net_device *dev, unsigned int txqueue)
{
	...
	/* e2cat - no call from Linux stack */
	BUG();
...
static void ec_poll(struct net_device *netdev)
{
...
#if 0
	link  = netif_carrier_ok(netdev) ? 1 : 0;
	if (link != prev_link) {
	...
	ring = &priv->rx_rings[DESC_INDEX];
	bcmgenet_desc_rx(ring, UINT_MAX);

	bcmgenet_tx_reclaim(netdev, &priv->tx_rings[DESC_INDEX]);
}
...
static int bcmgenet_probe(struct platform_device *pdev)
{
	...
	/* e2cat */
	pr_info("Broadcom (Gigabit Ethernet) controller driver - EtherCAT\n");
	...
	/* e2cat - check for etherCAT device (based on MAC) */
...
static int bcmgenet_suspend(struct device *d)
{
	...
	/* e2cat - no POWER management in e2cat */
	BUG();
