...
wlan_init_priv(pmlan_private priv)
{
	...
	priv->usr_dot_11ac_opermode_bw = 0;
	priv->usr_dot_11ac_opermode_nss = 0;
...
/* SOO.tech */
void wlan_reset_chip(void) {
	void *gpio_reg;
	int tmp;

	printk("%s: resetting the Marvell Wifi chipset...\n", __func__);
	/* reset mwifiex chip */
	gpio_reg = ioremap(0x01f02c00, 0x200);

	/* set line to 0 */
	tmp = ioread32(gpio_reg + 0x10);
	tmp &= ~(0x1 << 6);
	iowrite32(tmp, gpio_reg + 0x0);

	/* direction output */
	tmp = ioread32(gpio_reg + 0x0);
	tmp &= ~(0x5 << 24);
	tmp |= (0x1 << 24);
	iowrite32(tmp, gpio_reg + 0x0);
