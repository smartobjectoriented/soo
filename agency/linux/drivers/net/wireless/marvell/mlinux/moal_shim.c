...
#define write_can_lock(x)		((((arch_rwlock_t)((x)->raw_lock)).lock) == 0)
	    ...
	    IN t_u32 size, IN t_u32 flag, OUT t_u8 **ppbuf)
{
	...
	t_u32 mem_flag = (in_interrupt() || irqs_disabled() || !write_can_lock(&dev_base_lock)) ? GFP_ATOMIC : GFP_KERNEL;
...
moal_recv_event(IN t_void *pmoal_handle, IN pmlan_event pmevent)
{
	...
	case MLAN_EVENT_ID_FW_BG_SCAN_STOPPED:
		...
		if (IS_STA_CFG80211(cfg80211_wext)) {
			if (priv->sched_scanning) {
				cfg80211_sched_scan_stopped(priv->wdev->wiphy, 0); /* paravirt: +0 */
		...
		if (IS_STA_CFG80211(cfg80211_wext)) {
			if (priv->sched_scanning &&
				...
				woal_inform_bss_from_scan_result(priv, NULL,
								 MOAL_NO_WAIT);
				cfg80211_sched_scan_results(priv->wdev->wiphy, 0); /* paravirt: +0 */
				priv->last_event = 0;
				PRINTM(MEVENT,
...
#endif
		break;
	case MLAN_EVENT_ID_DRV_ROAM_OFFLOAD_ENABLE:
#ifdef STA_CFG80211
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 14, 0)
