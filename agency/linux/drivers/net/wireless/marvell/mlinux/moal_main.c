	...
	for_each_property_of_node(dt_node, prop) {
		if (!strncmp(prop->name, "drv_mode", strlen("drv_mode"))) {
			...
			if (!of_property_read_u32(dt_node, prop->name, &data)) {
			   ...
			   (prop->name, "hs_multi_dtim",
			    strlen("hs_multi_dtim"))) {
			if (!of_property_read_u32(dt_node, prop->name, &data)) {
				hs_multi_dtim = data;
				PRINTM(MIOCTL, "hs_multi_dtim=%d\n",
				       hs_multi_dtim);
			}
		} else if (!strncmp
...
woal_set_user_init_data(moal_handle *handle, int type, t_u8 wait_option)
{
	...
	if (handle->user_data) {
		...
		if (type == TXPWRLIMIT_CFG_DATA || type == INIT_HOSTCMD_CFG_DATA
...
woal_add_card_dpc(moal_handle *handle)
{
	...
	if (pmic
		...
		if (MLAN_STATUS_SUCCESS !=
		    woal_pmic_configure(handle, MOAL_CMD_WAIT)) {
	...
	if (antcfg
		...
		if (MLAN_STATUS_SUCCESS !=
		    woal_set_user_antcfg(handle, MOAL_CMD_WAIT)) {
...
woal_init_fw_dpc(moal_handle *handle)
{
	...
	if (cal_data_cfg && strncmp(cal_data_cfg, "none", strlen("none"))) {
		...
		if (handle->card_type == CARD_TYPE_SD8887 ||
		    ...
		    handle->card_type == CARD_TYPE_SD8987) {
			PRINTM(MERROR,
			       "Please add cal_data_cfg for 8887/8977/8997/8987\n");
			ret = MLAN_STATUS_FAILURE;
			goto done;
		}
	}
...
woal_open(struct net_device *dev)
{
	...
	/* On some systems the device open handler will be called before HW
	   ready. Use the following flag check and wait function to work
	   around the issue. */
	{
		int i = 0;
...
woal_dump_mlan_hex(moal_private *priv, t_u8 *buf, struct file *pfile)
{
	...
	if (!buf || !priv || !pfile ||
	    woal_get_debug_info(priv, MOAL_CMD_WAIT, &info)) {
		PRINTM(MMSG, "%s: can't retreive info\n", __func__);
		LEAVE();
		return 0;
	}
...
woal_request_fw_reload(moal_handle *handle, t_u8 mode)
{
	...
	for (intf_num = 0; intf_num < handle->priv_num; intf_num++) {
		woal_init_priv(handle->priv[intf_num], MOAL_PROC_WAIT);
#if defined(WIFI_DIRECT_SUPPORT)
#if defined(STA_SUPPORT) && defined(UAP_SUPPORT)
#if defined(STA_WEXT) || defined(UAP_WEXT)
