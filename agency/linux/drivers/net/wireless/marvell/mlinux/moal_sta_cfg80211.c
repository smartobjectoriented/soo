...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 31) || defined(COMPAT_WIRELESS)
...
/* paravirt: +reqid */
int woal_cfg80211_sched_scan_stop(struct wiphy *wiphy, struct net_device *dev, u64 reqid);
#endif
...
#endif
#endif
	t_u8 wait_option = MOAL_CMD_WAIT;
	int ret = 0;
	long timeout = 0;
			  ...
			  int idx, struct survey_info *survey)
{
	 ...
	 * SURVEY_INFO_CHANNEL_TIME_BUSY > NL80211_SURVEY_INFO_CHANNEL_TIME
	 * channel_time > time
	 *
	 */
#if 0
...
#endif /* 0 */
	survey->filled |= NL80211_SURVEY_INFO_CHANNEL_TIME | NL80211_SURVEY_INFO_CHANNEL_TIME_BUSY;
			  ...
			  struct cfg80211_chan_def *chandef)
{
	...
	if (chan_info.band_config.chanWidth == CHAN_BW_80MHZ)
		chan_info.center_chan = channel.center_chan;
...
/* paravirt: +reqid */
int
woal_cfg80211_sched_scan_stop(struct wiphy *wiphy, struct net_device *dev, u64 reqid)
{
	moal_private *priv = (moal_private *)woal_get_netdev_priv(dev);
	ENTER();
...
woal_register_cfg80211(moal_private *priv)
{
...
#endif
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 15, 0)
	if (fw_info.max_p2p_conn)
		wiphy->max_ap_assoc_sta = fw_info.max_p2p_conn;
#endif
#endif
	...
	wiphy->flags = 0;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 33) || defined(COMPAT_WIRELESS)
