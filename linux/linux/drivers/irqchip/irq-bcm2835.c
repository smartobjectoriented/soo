...
/* SOO.tech */
volatile int __irq_in_process = -1;
...
/* SOO.tech */
static int armctrl_set_affinity_irq(struct irq_data *data, const struct cpumask *dest, bool force) {
	/* Affinity is set only from RT cpu */
...
/* SOO.tech */
extern void __ipipe_assign_chip_timer(ipipe_irqdesc_t *irqdesc);

static void __ipipe_assign_chip(ipipe_irqdesc_t *irqdesc) {

	if ((__xntimer_rt != NULL) && (irqdesc->irq == __xntimer_rt->irq)) {
		__ipipe_assign_chip_timer(irqdesc);
		return ;
	}

	irqdesc->irq_data.chip = &armctrl_chip;
...
/* SOO.tech */
extern void smp_irq_handle(int cpu);
