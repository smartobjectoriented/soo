...
uint32_t *new_l1pgtable(void) {
	 ...
	 * 0x1000 is the top (not-included) limit of the PTE index for the level one page table.
	 */
	for (i = HYPERVISOR_VIRT_ADDR_DBG >> L1_PAGETABLE_SHIFT; i < 0x1000; i++)
	...
	for (i = 0; i < L1_PAGETABLE_ENTRIES; i++) {
		...
		if (*l1pte) {
			...
			if ((*l1pte & L1DESC_TYPE_MASK) == L1DESC_TYPE_PT) {
				for (j = 0; j < 256; j++) {
#ifdef CONFIG_SO3VIRT
					if (((i << 20) | (j << 12)) >= HYPERVISOR_VIRT_ADDR_DBG) {
#if 0
						lprintk("      (Mapped within the AVZ hypervisor, addr = 0x%x\n", (i << 20) | (j << 12));
#endif
						continue ;
					}
#endif /* CONFIG_SO3VIRT */
