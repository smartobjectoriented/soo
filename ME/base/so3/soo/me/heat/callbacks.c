...
int cb_pre_activate(soo_domcall_arg_t *args) {
	...
	sh_heat->me_common.here = agency_ctl_args.u.agencyUID;
	DBG(">> ME %d: Agency UID %d\n", ME_domID(), sh_heat->me_common.here);
...
int cb_cooperate(soo_domcall_arg_t *args) {
	...
	sh_weatherstation_t *incoming_sh_weatherstation;
	static uint64_t weatherstation_timestamp = 0;
	uint32_t pfn;
...
int cb_localinfo_update(void) {
	...
	DBG("SOO.heat cb_localinfo_update\n");
	return 0;
}
...
int cb_force_terminate(void) {
	DBG("SOO.heat cb_force_terminate\n");
...
void callbacks_init(void) {
	...
	// /* Allocate localinfo */
	sh_heat = (sh_heat_t *) get_contig_free_vpages(1);
