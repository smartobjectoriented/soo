...
int cb_cooperate(soo_domcall_arg_t *args) {
	...
	case COOPERATE_TARGET:
		...
		if(shared_data->type != incoming_sh_data->type){
			...
			/*if the ME come for get data*/
			if(incoming_sh_data->type == SOO_SIM_BENCH){
				incoming_sh_ledctrl->incoming_nr = sh_ledctrl->local_nr;

				/*reset the liste and comebacke*/
		        incoming_sh_data->nb_device_visited = 1;
				memcpy(&incoming_sh_data->ID_device_visited[0], &agency_ctl_args.u.agencyUID_args.agencyUID.id, SOO_AGENCY_UID_SIZE);
				incoming_sh_data->comeback = true;
			}
...
void callbacks_init(void) {
	...
	shared_data->type = SOO_SIM_OUTDOOR;
