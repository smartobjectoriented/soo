...
const char ID_TARGET[] = {0x8d,0x77,0x9c,0xd0,0x82,0xbb,0x25,0x20,0xbf,0x10,0xe6,0xee,0x4d,06,0x7f,0x96};
...
int cb_cooperate(soo_domcall_arg_t *args) {
	...
	case COOPERATE_TARGET:
		...
		/*check if ME is comeback home*/
		if(incoming_sh_data->comeback == true && 
		!memcmp(&incoming_sh_data->id,&agency_ctl_args.u.agencyUID_args.agencyUID.id,SOO_AGENCY_UID_SIZE)){
			/*get the data from target*/
			lprintk("ME comebacke from soo.outdoor !!");
			sh_ledctrl->incoming_nr = incoming_sh_data->ledctrl.incoming_nr;
		...
		/*for a ME using target check the comback bit*/
		if(incoming_sh_data->comeback == true && shared_data->comeback == false){
			/*kill the target ME*/
			agency_ctl_args.cmd = AG_FORCE_TERMINATE;
			agency_ctl_args.slotID = ME_domID();
			args->__agency_ctl(&agency_ctl_args);
		...
		if(incoming_sh_data->comeback == false && shared_data->comeback == true){
			/*kill the target ME*/
			agency_ctl_args.cmd = AG_FORCE_TERMINATE;
			agency_ctl_args.slotID = incoming_sh_data->slotID;
			args->__agency_ctl(&agency_ctl_args);
...
void callbacks_init(void) {
	...
	/*set the target*/
    memcpy(&shared_data->targetID, &ID_TARGET, SOO_AGENCY_UID_SIZE);
	shared_data->type = SOO_SIM_BENCH;
