...
static void vknx_probe(struct vbus_device *vdev) {
	...
	res = vbus_grant_ring(vdev, phys_to_pfn(virt_to_phys_pt((uint32_t) vknx_priv->vknx.ring.sring)));
	if (res < 0)
		BUG();
...
static void vknx_closed(struct vbus_device *vdev) {
	...
	if (vknx_priv->vknx.ring_ref != GRANT_INVALID_REF) {
		...
		free_vpage((uint32_t) vknx_priv->vknx.ring.sring);
...
static int vknx_init(dev_t *dev) {
	vknx_priv_t *vknx_priv;
