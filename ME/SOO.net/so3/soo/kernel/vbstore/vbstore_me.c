...
void remove_vbstore_entries(void) {
	...
	fdt_node = fdt_find_compatible_node("vnet,frontend");
	if (fdt_device_is_available(fdt_node)) {
		DBG("%s: removing vnet from vbstore...\n", __func__);
		vbstore_dev_remove(ME_domID(), "vnet");
	}
}
...
void vbstore_devices_populate(void) {
	...
	if (fdt_device_is_available(fdt_node)) {
        ...
        if (fdt_device_is_available(fdt_node)) {
                DBG("%s: init vnet...\n", __func__);
                vbstore_dev_init(ME_domID(), "vnet", false, "vnet,frontend");
        }
