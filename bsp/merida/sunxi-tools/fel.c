...
static bool verbose = true; /* If set, makes the 'fel' tool more talkative */
...
void *load_file(const char *name, size_t *size)
{
	...
	/* SOO.tech */
	/* Add an "artificial" '\0' at the end of the file.
	 * For importing uEnv.txt in U-boot for example, a binary '\0'
	 * is required at the end of the file, which is not really present
	 * in the file itself.
	 */
	buf[offset++] = 0;
		...
		uint16_t vendor_id, uint16_t product_id)
{
	...
	if (busnum < 0 || devnum < 0) {
		 ...
		 */
		result = libusb_open_device_with_vid_pid(NULL, vendor_id & 0xffff, product_id & 0xffff);
		if (!result) {
			switch (errno) {
			case EACCES:
				...
				break;
			default:
				fprintf(stderr, "ERROR: Allwinner USB FEL device not found! err=%i\n",errno);
 err(1, "ERROR: Allwinner USB FEL device not found!");
				break;
			}
			exit(1);
		}
